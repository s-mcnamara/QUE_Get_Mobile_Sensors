...
public class SensorActivity extends Activity implements SensorEventListener {
		...
		// Open output file, if all is successful, call MonitorSensor()
		 if (DisplayMessageActivity.isExternalStorageWritable()){
			    ...
			    if (!dirObj.exists()) {
			      ...
			      String FileName = new String("temperatureoutput.txt") ;
			      //FileNum++ ;
			      Log.i(TAG,FileName) ;
				  ...
				  Date Now = new Date() ;
				  String OutStr = String.format(Locale.US,"Starting Temperature Sensor Monitor at %s\n",Now.toString()) ;
				  Log.i(TAG,OutStr) ;
				  try {
	...
	private void MonitorSensor() {
	    ...
	    mSensor = mSensorManager.getDefaultSensor(7);
	    ...
	    public final void onSensorChanged(SensorEvent event) {
	      ...
	      // write data every 300 times the function is called
	      if (++CallCount == 300) { 
	    	  if (OKtoWrite){
	            ...
	            String OutStr = String.format(Locale.US,"Time: %s Temp: %4.2fC\n",TimeString,AmbientTemp) ;
	            //Log.i(TAG,String.format("Time: %d Temp: %4.2fC",EvtDate,AmbientTemp)) ; 
	            CallCount = 0 ;
	            Log.i(TAG,OutStr) ; 
	            try {
